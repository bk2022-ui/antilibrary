# Manthan — Product Definition

## What it is

A personal library service for discerning people who want to build a great library. Not large — well-curated, intentional, shaped by genuine intellectual threads.

Manthan reads a library as a mind. It surfaces what's already there that the owner couldn't see because they were too close to it. Then it maps where the library wants to go next.

---

## The two deliverables

**For what exists — hidden threads**
The map of what's already there. The library churned, and what surfaces surprises the owner. Threads they were building without knowing it. Clusters with gravity they hadn't named. The founding figures their thinking orbits. The core tension they've been holding in two hands without realizing it.

**For what comes next — a path**
Not a reading list. A path — with direction and intention. Gaps named. Next books chosen because they extend a real thread, break open a cluster that hasn't resolved, or introduce the adversary a one-sided thread is missing.

The two together are the product. Hidden threads give insight. The path gives agency. One looks back, one looks forward.

---

## Who it's for

People who already sense their library has a shape they can't quite see. They know they're drawn to certain ideas across domains. They know some books belong together but can't articulate why. They've been building something without knowing exactly what.

Manthan names it for them.

**Not for everyone.** For people who:
- Already have a library worth analysing — not beginners
- Care about quality of connection, not quantity of books
- Want to be challenged, not just organised
- Would immediately reject shallow connections
- Take their libraries seriously as a record of how they think

---

## The service structure

**Session one — Ingest and Analyse**
Collect the library in any form: shelf photographs, Goodreads export, Kindle list, conversation. Run Manthan — Collect, Classify, Churn, surface Threads and Clusters. Prime with the owner to catch what the algorithm missed.

**The Deliverable**
A written document. The threads, the clusters, the reveal — written, not just diagrammed. Because the owner needs to recognise themselves in it. A diagram can be ignored. A sentence that names your hidden intellectual obsession cannot.

**Session two — The Path**
Given what was found: where does this library want to go? Which gaps matter? Which threads need their anchor book? What are the next ten books that would make this library significantly more itself?

---

## The method underneath

The Manthan Library System — an Adirondacks search, not a Mount Fuji climb. Many lenses, diverse starting points, random probes, human priming. No single peak, no linear path to a correct answer.

Full method documented in `LIBRARY-SYSTEM.md`.

---

## What makes it original

Nobody has built a system that:
1. Reads a library as a mind rather than a catalogue
2. Runs a multi-lens Adirondacks search across the collection
3. Surfaces hidden threads the owner didn't know they were building
4. Converses with the owner to prime what the algorithm missed
5. Delivers a path forward, not just an analysis of the past

The conversation is the product. The visualisation is one output of it.
